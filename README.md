...
- 実行中の仮想C:をベースHDDとsavから統合し、他のVMへ移行できる単体raw IMGとして安全に書き出し
...
├── Exports/
│   └── win-merged-YYYYMMDD-HHMMSS-SSS.img # ベースHDDとsavを統合した移行用raw IMG
├── Saves/
...
他の仮想マシンへ移行するときは、`共有`画面の「移行用の統合HDDを作成…」を選びます。Windowsを一時停止して最新の差分をflushした後、現在の仮想C:をセクタ単位で`Win9x/Exports`へ新しいraw `.img`として書き出し、完了時にiOSの共有メニューを開きます。元の`.img`/`.vhd`と`.sav`は変更されません。出力には仮想HDD全容量ぶんの空き領域が必要で、処理中はアプリを前面に保ってください。出力IMGはUTM/QEMUなどでraw IDEディスクとして接続でき、必要なら移行先のツールでqcow2・VHD・VDI等へ変換できます。
...
- `共有` → 「移行用の統合HDDを作成…」: ベースHDDとsavの現在内容を、他のVMへ持ち出せる単体raw IMGとして`Win9x/Exports`へ保存します。元データは上書きしません。
